...
class TestRecommendationEngine(unittest.TestCase):
    ...
    def test_calculate_composite_score_with_cosine_similarity(self):
        """Test composite score calculation including cosine similarity metrics."""
        # Create sample recommendations DataFrame with cosine similarity
        recommendations = pd.DataFrame({
            'recipe_id': [1, 2, 3, 4],
            'name': ['Recipe 1', 'Recipe 2', 'Recipe 3', 'Recipe 4'],
            'jaccard': [0.8, 0.6, 0.4, 0.3],
            'cosine': [0.9, 0.7, 0.5, 0.8],
            'score': [0.7, 0.8, 0.6, 0.5]
        })
        ...
        # Verify that cosine similarity influences the composite score
        # Recipe with high cosine should get bonus points
        high_cosine_recipe = result[result['cosine'] == result['cosine'].max()]
        self.assertGreater(len(high_cosine_recipe), 0)
        
        # Verify all composite scores are reasonable (between 0 and 1)
        self.assertTrue((result['composite_score'] >= 0).all())
        self.assertTrue((result['composite_score'] <= 1).all())
        
        # Verify that cosine similarity contributes to composite score
        # Recipe 1 has highest cosine (0.9), should have higher composite score than recipe 3 (cosine 0.5)
        recipe1_score = result[result['recipe_id'] == 1]['composite_score'].iloc[0]
        recipe3_score = result[result['recipe_id'] == 3]['composite_score'].iloc[0]
        self.assertGreater(recipe1_score, recipe3_score)

    def test_calculate_composite_score_with_missing_columns(self):
        """Test composite score calculation with missing jaccard, cosine or score columns."""
        ...
        # Test with missing cosine column
        recommendations_no_cosine = pd.DataFrame({
            'recipe_id': [1, 2, 3],
            'name': ['Recipe 1', 'Recipe 2', 'Recipe 3'],
            'jaccard': [0.8, 0.6, 0.4],
            'score': [0.9, 0.7, 0.5]
        ...
        result = RecommendationEngine._calculate_composite_score(recommendations_no_cosine)
        self.assertIn('composite_score', result.columns)
...
class TestRecommendationEnginePytest:
    ...
    def test_hybrid_scoring_weights(self):
        """Test that hybrid scoring properly weights Jaccard, Cosine, Rating and Popularity."""
        # Create test data with clear differences
        test_df = pd.DataFrame({
            'recipe_id': [1, 2, 3],
            'jaccard': [1.0, 0.0, 0.5],  # Recipe 1 has highest Jaccard
            'cosine': [0.0, 1.0, 0.5],   # Recipe 2 has highest Cosine  
            'score': [0.5, 0.5, 1.0],    # Recipe 3 has highest Rating/Popularity
        })
        
        result = RecommendationEngine._calculate_composite_score(test_df)
        
        # Verify composite scores are reasonable (between 0 and 1)
        assert result['composite_score'].min() >= 0
        assert result['composite_score'].max() <= 1
        
        # With 60% normalized_score + 25% Jaccard + 15% Cosine weighting,
        # Recipe 3 should have highest composite score due to high normalized_score weight (60%)
        highest_composite = result.loc[result['composite_score'].idxmax(), 'recipe_id']
        assert highest_composite == 3  # Recipe with highest score component
        
        # Verify that Jaccard and Cosine similarity both contribute to the composite score
        recipe1_score = result[result['recipe_id'] == 1]['composite_score'].iloc[0]  # High Jaccard
        recipe2_score = result[result['recipe_id'] == 2]['composite_score'].iloc[0]  # High Cosine
        
        # Both should be > 0 due to their respective high similarity scores
        assert recipe1_score > 0
        assert recipe2_score > 0

    def test_cosine_similarity_bonus_system(self):
        """Test that cosine similarity bonus system works correctly."""
        # Test data where cosine similarity should provide bonus
        test_df = pd.DataFrame({
            'recipe_id': [1, 2, 3],
            'jaccard': [0.5, 0.5, 0.5],  # Same Jaccard for fair comparison
            'cosine': [0.9, 0.2, 0.4],   # Recipe 1 has high cosine (>0.3), Recipe 3 moderate, Recipe 2 low
            'score': [0.5, 0.5, 0.5],    # Same base score
        })
        
        result = RecommendationEngine._calculate_composite_score(test_df)
        
        # Recipe 1 should have higher composite score due to high cosine similarity
        recipe1_score = result[result['recipe_id'] == 1]['composite_score'].iloc[0]
        recipe2_score = result[result['recipe_id'] == 2]['composite_score'].iloc[0]
        recipe3_score = result[result['recipe_id'] == 3]['composite_score'].iloc[0]
        
        # Recipe 1 (high cosine) should score higher than Recipe 2 (low cosine)
        assert recipe1_score > recipe2_score
        # Recipe 3 (moderate cosine) should score higher than Recipe 2 (low cosine)  
        assert recipe3_score > recipe2_score
